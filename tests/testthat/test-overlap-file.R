...
gr_exon_structure_test <- GRanges(
  ...
  ranges = IRanges(
    start = c(1000, # gene_1
              ...
              2000, 2000, 3000, 4000), # tx_3 (three exons)
    end = c(5000, # gene_1
            5000, 5000, # tx_1
            5000, 2500, 5000, # tx_2
            5000, 2500, 3500, 5000) # tx_3
  ),
  type = c("gene",  
           "transcript", "exon", 
           "transcript", "exon", "exon", 
...
test_that("identical transcripts are collapsed after truncation, positive strand 3'", {
  ...
  for (n in LENGTHS_TO_TEST) {
    ...
    test_overlap <- data.frame(queryTx = "tx_2", subjectTx = "tx_1", queryGene = "gene_1", subjectGene = "gene_1")
    read_overlap <- read.table(temp_file, sep = "\t", header = T)[, c("queryTx", "subjectTx", "queryGene", "subjectGene")]
...
test_that("identical transcripts are collapsed after truncation, positive strand 5'", {
  ...
  for (n in LENGTHS_TO_TEST) {
    ...
    ## correct overlaps removal
    test_overlap <- data.frame(queryTx = logical(0), subjectTx = logical(0), queryGene = logical(0), subjectGene = logical(0))
    read_overlap <- read.table(temp_file, sep = "\t", header = T)[, c("queryTx", "subjectTx", "queryGene", "subjectGene")]
...
test_that("transcripts with same coordinates but different exon structure", {
  ...
  for (n in LENGTHS_TO_TEST) {
    ...
    # Read overlap file
    read_overlap <- read.table(temp_file, sep = "\t", header = TRUE)[, c("queryTx", "subjectTx", "queryGene", "subjectGene")]
    
    # Should detect 3 overlaps (tx_2 vs tx_1, tx_3 vs tx_1 and tx_3 vs tx_2)
    expect_equal(nrow(read_overlap), 3)
    
    # All overlaps should be within gene_1
    expect_true(all(read_overlap$queryGene == "gene_1"))
    expect_true(all(read_overlap$subjectGene == "gene_1"))
