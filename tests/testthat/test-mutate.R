...
txdb_multi_tx <- txendcutr:::.suppressTxDbGenomeWarning(makeTxDbFromGRanges(gr_multi_tx))
...
test_that("adds correct transcript names, multi-exon", {
  grl <- txendcutr:::.mutateEach(grl_multi_exon, new_col = names(grl_multi_exon))
  for (tx in names(grl)) {
...
test_that("adds correct transcript names, multi-tx", {
  grl <- txendcutr:::.mutateEach(grl_multi_tx, new_col = names(grl_multi_tx))
  for (tx in names(grl)) {
    expect_setequal(grl[[tx]]$new_col, tx)
  }
})
...
test_that("SimpleGRangesList and CompressedGRangesList match", {
  ...
  sgrl_out <- txendcutr:::.mutateEach(sgrl_in, new_col = names(sgrl_in))
  cgrl_out <- txendcutr:::.mutateEach(cgrl_in, new_col = names(cgrl_in))
...
test_that("incorrect assignment lengths are caught", {
  ...
  empty_col <- numeric(0)
  expect_error(txendcutr:::.mutateEach(grl_multi_tx, new_col = bad_col_long))
  expect_error(txendcutr:::.mutateEach(grl_multi_tx, new_col = bad_col_short))
  expect_error(txendcutr:::.mutateEach(grl_multi_tx, new_col = empty_col))
})
