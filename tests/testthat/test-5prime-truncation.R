...
txdb_single_contig_w1000 <- txendcutr:::.suppressTxDbGenomeWarning(makeTxDbFromGRanges(gr_single_contig_w1000))

## Negative Strand
txdb_single_contig_w1000_neg <- txendcutr:::.suppressTxDbGenomeWarning(makeTxDbFromGRanges(invertStrand(gr_single_contig_w1000)))
...
gr_multi_exon <- GRanges(
  ...
  strand = "+",
  ranges = IRanges(
    start = c(1000, 1000, 1000, 5000, 9000),
...
txdb_multi_exon <- txendcutr:::.suppressTxDbGenomeWarning(makeTxDbFromGRanges(gr_multi_exon))

## Negative Strand
txdb_multi_exon_neg <- txendcutr:::.suppressTxDbGenomeWarning(makeTxDbFromGRanges(invertStrand(gr_multi_exon)))
...
test_that("simple truncation works, positive strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate5primeTxome(txdb_single_contig_w1000, maxTxLength = n, quiet = TRUE)
...
test_that("simple truncation works, negative strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate5primeTxome(txdb_single_contig_w1000_neg, maxTxLength = n, quiet = TRUE)
...
test_that("idempotent", {
  txdb_res_w500_1 <- truncate5primeTxome(txdb_single_contig_w1000, maxTxLength = 500, quiet = TRUE)
  txdb_res_w500_2 <- truncate5primeTxome(txdb_res_w500_1, maxTxLength = 500, quiet = TRUE)
...
test_that("spliced truncation works, positive strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate5primeTxome(txdb_multi_exon, maxTxLength = n, quiet = TRUE)
...
test_that("spliced truncation works, negative strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate5primeTxome(txdb_multi_exon_neg, maxTxLength = n, quiet = TRUE)
