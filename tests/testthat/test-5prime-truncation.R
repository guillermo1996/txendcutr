...
test_that("simple truncation works, positive strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncateTxome(txdb_single_contig_w1000, maxTxLength=n, txEnd="5prime")
...
test_that("simple truncation works, negative strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncateTxome(txdb_single_contig_w1000_neg, maxTxLength=n, txEnd="5prime")
...
test_that("idempotent", {
  txdb_res_w500_1 <- truncateTxome(txdb_single_contig_w1000, maxTxLength=500, txEnd="5prime")
  txdb_res_w500_2 <- truncateTxome(txdb_res_w500_1, maxTxLength=500, txEnd="5prime")
...
test_that("spliced truncation works, positive strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncateTxome(txdb_multi_exon, maxTxLength=n, txEnd="5prime")
...
test_that("spliced truncation works, negative strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncateTxome(txdb_multi_exon_neg, maxTxLength=n, txEnd="5prime")
...
test_that("simple truncation works with wrapper, positive strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate5primeTxome(txdb_single_contig_w1000, maxTxLength=n)
