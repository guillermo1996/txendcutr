...
test_that("simple truncation works, positive strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate3primeTxome(txdb_single_contig_w1000, maxTxLength = n, quiet = TRUE)
...
test_that("simple truncation works, negative strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate3primeTxome(txdb_single_contig_w1000_neg, maxTxLength = n, quiet = TRUE)
...
test_that("idempotent", {
  txdb_res_w500_1 <- truncate3primeTxome(txdb_single_contig_w1000, maxTxLength = 500, quiet = TRUE)
  txdb_res_w500_2 <- truncate3primeTxome(txdb_res_w500_1, maxTxLength = 500, quiet = TRUE)
...
test_that("spliced truncation works, positive strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate3primeTxome(txdb_multi_exon, maxTxLength = n, quiet = TRUE)
...
test_that("spliced truncation works, negative strand", {
  ...
  for (n in LENGTHS_TO_TEST) {
    txdb_res <- truncate3primeTxome(txdb_multi_exon_neg, maxTxLength = n, quiet = TRUE)
