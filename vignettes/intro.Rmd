---
title: "Introduction to txendcutr"
author:
  ...
  - name: Mervin M Fansler
    ...
    comment: "Original author of txcutr, from which this package is forked"
output:
  BiocStyle::html_document:
    ...
    code_folding: show
date: "`r doc_date()`"
package: "`r pkg_ver('txendcutr')`"
vignette: >
  %\VignetteIndexEntry{Introduction to txendcutr}
...
bib <- c(
    ...
    txendcutr = citation("txendcutr")[1]
)
```
...
`txendcutr` is a standalone fork of [`txcutr`](https://github.com/mfansler/txcutr) that adds 5' truncation support, an optimized truncation pipeline, and an expanded test suite. See the package `README` for a full list of differences from upstream `txcutr`.

## Install `txendcutr`

`R` is an open-source statistical environment which can be easily modified to enhance its functionality via packages. `txendcutr` is not published on Bioconductor or CRAN; it is distributed as a GitHub package built on top of the Bioconductor infrastructure. `R` can be installed on any operating system from [CRAN](https://cran.r-project.org/), after which you can install `txendcutr` by using the following commands in your `R` session:
...
## installs from GitHub, resolving Bioconductor dependencies correctly
BiocManager::install("guillermo1996/txendcutr")
```

## Required Background

`txendcutr` is based on many other packages and in particular on those that have implemented the infrastructure needed for dealing with transcriptome annotations. That is, packages like `r Biocpkg("GenomicRanges")` and `r Biocpkg("GenomicFeatures")`. While we anticipate most use cases for `txendcutr` to not require manipulating `GRanges` or `TxDb` objects directly, it would be beneficial to be familiar with the vignettes provided in these packages.

## Asking for Help

As package developers, we try to explain clearly how to use our packages and in which order to use the functions. But `R` and `Bioconductor` have a steep learning curve so it is critical to learn where to ask for help. For `txendcutr`-specific questions, bugs, or feature requests, please use the [GitHub issue tracker](https://github.com/guillermo1996/txendcutr/issues). For general questions about the underlying Bioconductor infrastructure (`GenomicRanges`, `GenomicFeatures`, `TxDb` objects, etc.), the [Bioconductor support site](https://support.bioconductor.org/) is a great resource. It is particularly critical that you provide a small reproducible example and your session information so package developers can track down the source of the error.

## Citing `txendcutr`

We hope that `txendcutr` will be useful for your research. Please use the following information to cite the package and the overall approach. Thank you!
...
# Quick Start to Using `txendcutr`
...
As seen from the above output, the transcript lengths are variable, but in a 3'-end tag-based RNA-sequencing library, we expect reads to only come from a narrow region at the 3' end. We can use the methods in the `txendcutr` package to restrict the annotation to that region for each transcript. 
...
By default, `truncateTxome` keeps the last (3') portion of each transcript, which suits 3'-end tag sequencing protocols. For protocols that instead read from the 5' end, `txendcutr` provides `truncate5primeTxome` as convenience wrappers around `truncateTxome`:

```{r "truncate_sgd_5prime"}
txdb_5p_w500 <- truncate5primeTxome(txdb, maxTxLength=500)

transcriptLengths(txdb_5p_w500)[1:10,]
```

Both wrappers accept a `quiet` argument to suppress progress messages, and an `overlapFile` argument to export a TSV listing which transcripts were collapsed as duplicates during truncation (see `?truncateTxome` for details):

```{r "truncate_sgd_overlap", eval=FALSE}
overlap_file <- tempfile("sacCer3_chrI.sgd.overlaps", fileext=".tsv")
txdb_w500_quiet <- truncate3primeTxome(txdb, maxTxLength=500,
                                        overlapFile=overlap_file, quiet=TRUE)
```
...
gtf_file <- tempfile("sacCer3_chrI.sgd.txendcutr_w500", fileext=".gtf.gz")
...
Algorithmically, `txendcutr` will give preference to more distal transcripts in the merge assignment. For example, looking at transcripts from gene **YAL026C**:
...
### Making TxDb Objects
The `r Biocpkg("txdbmaker")` package provides a number of means to create custom `TxDb` objects that can then be used by `txendcutr`. Alternative workflows might include starting from a GTF/GFF3 transcriptome annotation and using the `txdbmaker::makeTxDbFromGFF` method, or using the `txdbmaker::makeTxDbFromBiomart` method to create the object from Biomart resources.
...
We encourage use of a parallel parameter option, such as `MulticoreParam`. Please ensure adequate memory *per core* is available.
...
The `txendcutr` package `r Citep(bib[["txendcutr"]])` (forked from `r Biocpkg("txcutr")`) was made possible thanks to:
